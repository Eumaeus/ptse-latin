...
"""
  ...
  "The PtseLatin JS build" should "distinguish between characters based on Unicode codepoints" in {
  ...
  it should "distinguish between characters based on Unicode codepoints after NFC normalization" in {
    ...
    assert (longA.normalize(scala.scalajs.js.UnicodeNormalizationForm.NFC) == composedLongA.normalize(scala.scalajs.js.UnicodeNormalizationForm.NFC))
